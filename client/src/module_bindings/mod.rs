...
impl SpacetimeModule for Module {
    fn handle_table_update(
        ...
        match table_name {
            "Person" => client_cache
                .handle_table_update_no_primary_key::<person::Person>(callbacks, table_update),
            _ => {
    ...
    fn invoke_row_callbacks(
        ...
        reminders.invoke_callbacks::<person::Person>(worker, &reducer_event, state);
    }
    fn handle_event(
        &self,
        ...
        match &reducer_call.reducer_name[..] {
            "add" => _reducer_callbacks.handle_event_of_type::<add_reducer::AddArgs, ReducerEvent>(
                event,
                _state,
                ReducerEvent::Add,
            ),
            "say_hello" => _reducer_callbacks
    ...
    fn handle_resubscribe(
        ...
        match table_name {
            "Person" => {
                client_cache.handle_resubscribe_for_type::<person::Person>(callbacks, new_subs)
            }
            _ => {
                spacetimedb_sdk::log::error!("TableRowOperation on unknown table {:?}", table_name)
            }
